...
export function createLoadCommand(): Command {
  ...
  command
    ...
    .option("--host <host>", "Database host", undefined)
    .option("--port <port>", "Database listener port", Number.parseInt)
    .option("--service-name <name>", "Database service name", undefined)
    .option("--user <user>", "Database username", undefined)
    .option("--password <password>", "Database password", undefined)
    .option(
      "--connect-string <str>",
